...
class Lotto645(NumberBasedLottery):
    ...
    def __init__(self):
        ...
        self._logger.debug("6/45 로또 게임 객체 생성")
        super().__init__(
            name="로또 6/45",
            ticket_price=1000,
            min_number=1,
    ...
    def _setup_prize_structure(self):
        ...
        self._logger.debug("실제 6/45 로또 상금 구조 설정하기") 
    ...
    def create_ticket(self, numbers: Optional[List[int]] = None) -> Lotto645Ticket:
        self._logger.info("6/45 로또 티켓 만들기")
        if numbers is None:
            self._logger.debug("숫자가 없으므로 난수 생성")
            numbers = self.generate_random_numbers()
        
        ticket = Lotto645Ticket(numbers)
        if not ticket.validate():
            self._logger.debug("유효하지 않은 번호, ValueError 발생")
